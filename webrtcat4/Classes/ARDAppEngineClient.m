...
static NSString * kARDRoomServerJoinFormatLoopback;
...
@implementation ARDAppEngineClient
    ...
    - (instancetype)initWithUrlBase:(NSString *) urlBase
                             {
        if (self = [super init]) {
            kARDRoomServerHostUrl = urlBase;
            kARDRoomServerJoinFormat = [urlBase  stringByAppendingString:@"/join/%@"];
            kARDRoomServerJoinFormatLoopback = [urlBase  stringByAppendingString:@"/join/%@?debug=loopback"];
            kARDRoomServerMessageFormat = [urlBase  stringByAppendingString:@"/message/%@/%@"];
            kARDRoomServerLeaveFormat = [urlBase  stringByAppendingString:@"/leave/%@/%@"];
        }
...
- (void)leaveRoomWithRoomId:(NSString *)roomId
  ...
  [NSURLConnection sendAsyncRequest:request
                  completionHandler:^(NSURLResponse *response, NSData *data, NSError *e) {
                    if (e) {
                        NSLog(@"leaveRoomWithRoomId error");
