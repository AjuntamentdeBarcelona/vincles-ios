    ...
    fileprivate func addLinkAttribute(_ mutAttrString: NSMutableAttributedString) {
        ...
        for (type, elements) in activeElements {
            ...
            for element in elements {
                ...
                var add = true
                if element.range.location > 0{
                    let prevIndex = element.range.location - 1
                    let chr = mutAttrString.mutableString.character(at: prevIndex)
                    
                    if let unicode = UnicodeScalar(chr) {
                        let str2 =  Character(unicode)
                        if str2 == "@"{
                            add = false
                        }
                    }
