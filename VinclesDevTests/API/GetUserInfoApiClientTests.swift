//
//  GetUserInfoApiClientTests.swift
//  Vincles BCN
//
...
class GetUserInfoApiClientTests: XCTestCase {
    ...
    func testGetUserSuccess() {
        ...
        stub(condition: isHost(IP)) { _ in
            let stubPath = OHPathForFile("GetUserInfoResponseSuccess.json", type(of: self))
            return fixture(filePath: stubPath!,status: 201, headers: ["Content-Type":"application/json"])
        }
        
        let expectation = self.expectation(description: "calls the callback with a resource object")
        
        ApiClient.getUserSelfInfo(onSuccess: { (responseDict) in
