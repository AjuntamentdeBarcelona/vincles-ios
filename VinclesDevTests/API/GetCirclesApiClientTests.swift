//
//  GetCirclesApiClientTests.swift
//  Vincles BCN
//
...
class GetCirclesApiClientTests: XCTestCase {
    
    func testGetCirclesSuccess() {
        
        stub(condition: isHost(IP)) { _ in
            // Stub it with our "wsresponse.json" stub file (which is in same bundle as self)
            let stubPath = OHPathForFile("GetCirclesResponseSuccess.json", type(of: self))
            return fixture(filePath: stubPath!, headers: ["Content-Type":"application/json"])
        }
        
        let expectation = self.expectation(description: "calls the callback with a resource object")
        
        ApiClient.getCirclesUser( onSuccess: { (responseArray) in
