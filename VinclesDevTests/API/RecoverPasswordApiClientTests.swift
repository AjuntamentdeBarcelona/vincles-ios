//
//  RecoverPasswordApiClientTests.swift
//  Vincles BCN
//
...
class RecoverPasswordApiClientTests: XCTestCase {
    ...
    func testRecoverSuccess() {
        ...
        stub(condition: isHost(IP)) { _ in
            let stubData = "Requested recover password".data(using: String.Encoding.utf8)
            return OHHTTPStubsResponse(data: stubData!, statusCode:200, headers:nil)
        }
        
        let expectation = self.expectation(description: "calls the callback with a resource object")
        
        ApiClient.recoverPassword(username: "XXX", onSuccess: {
            expectation.fulfill()
        }) { (error) in
