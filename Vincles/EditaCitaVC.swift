...
class EditaCitaVC: VinclesVC, UITableViewDataSource,UITableViewDelegate,UITextViewDelegate {
        ...
        if descripCitaTextView.text != "" {
            ...
            VinclesApiManager.sharedInstance.updateAgendaEvent(parameters,idCalendar:Int(usrVincle.idCalendar!)!,idEvent:Int(currentCita!.id!)!,completion: {(status) in
                ...
                if status == "SUCCESS" {
                    ...
                    if self.currentCita?.state == EVENT_STATE_ACCEPTED {
                        
                        Utils().deleteEventInCalendar(self.currentCita!)
                    }
                    ...
                    self.currentCita?.state = EVENT_STATE_PENDING
    ...
    func setNavBar() {
        ...
        let imgData = Utils().imageFromBase64ToData(usrVincle.photo!)
        let xarxaImg = UIImage(data:imgData)
        let xarxaImgView = UIImageView(frame: CGRectMake(0, 0, 40, 40))
        
        xarxaImgView.contentMode = .ScaleAspectFill
        xarxaImgView.image = xarxaImg
